...
def _label_financial_level(income: float, expenses: float, savings: float, debt: float) -> str:
    # Simple, transparent rules to generate a training set.
    # The decision tree will learn these boundaries from synthetic data.
    if income <= 0:
        return "Critical"
...
def build_dataset(n: int = 5000, seed: int = 42):
    ...
    # Income distribution (monthly, INR-like scale but unit-agnostic)
    ...
    # Debt: skewed; correlated with overspending somewhat
